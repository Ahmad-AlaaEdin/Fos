...
void _main(void)
{
	...
	void* ptr_allocations[20] = {0};
	{
		...
		//2 MB
		{
			...
			if ((uint32) ptr_allocations[0] != (pagealloc_start)){ cprintf("expected %d real %d",pagealloc_start,(uint32) ptr_allocations[0]); panic("Wrong start address for the allocated space... ");}
		...
		//2 MB
		{
			...
			if ((uint32) ptr_allocations[1] != (pagealloc_start + 2*Mega)) panic("Wrong start address for the allocated space...expected:%lu  actual:%lu ",(pagealloc_start + 2*Mega),ptr_allocations[1]);
			if ((freeFrames - sys_calculate_free_frames()) >= 512) panic("Wrong allocation: pages are allocated in memory while it's not supposed to!");
			if ((sys_pf_calculate_allocated_pages() - usedDiskPages) != 0) panic("Extra or less pages are allocated in PageFile");
