	...
	if ((faulted_env->env_page_directory[PDX(fault_va)] & PERM_PRESENT)
		...
		if (userTrap) {
			...
			int perms = pt_get_page_permissions(faulted_env->env_page_directory,
					fault_va);
			// page exits but read only
			if ((perms & ~PERM_WRITEABLE) && (perms & PERM_PRESENT)) {
				cprintf("\nread only\n");
				sched_kill_env(faulted_env->env_id);
			}

			//check if in user heap
			if (fault_va >= USER_HEAP_START && fault_va < USER_HEAP_MAX) {

				cprintf("\n in user heap\n");
				uint32 * ptr_page_table = NULL;
				int ret = get_page_table(faulted_env->env_page_directory,
						fault_va, &ptr_page_table);
				unsigned int table_entry = ptr_page_table[PTX(fault_va)];
				// kill if pointing unmarked page
				if (table_entry == 0) {
					cprintf("\npointing page unmarked in user heap %lu\n",fault_va);
					sched_kill_env(faulted_env->env_id);
				}
			}
