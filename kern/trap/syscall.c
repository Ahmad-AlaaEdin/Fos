...
struct allocInfo allocArr[1024*1024];
uint32 startAdd = curenv->limitAddr+PAGE_SIZE;
bool checkList[(USER_HEAP_MAX-curenv->limitAddr)/PAGE_SIZE] = {0};
*/
int sys_get_perm(uint32 va){
	return pt_get_page_permissions(curenv->env_page_directory, va);
...
uint32 sys_get_hard_limit()
{
	...
	return (uint32)curenv->limitAddr;
}
void* sys_sbrk(int increment)
{
	...
	if(increment == 0 )
		...
		if (env->sbrkAddr + (np * PAGE_SIZE) < env->limitAddr) {
			if(env->sbrkAddr>=USER_HEAP_START&&env->sbrkAddr<USER_HEAP_MAX){cprintf("\n**********in user memory**********\n ");}
			if(env->sbrkAddr>=KERNEL_HEAP_START&&env->sbrkAddr<KERNEL_HEAP_MAX){cprintf("\n**********in kernel memory*************\n ");}
			sys_allocate_user_mem(env->sbrkAddr,(np * PAGE_SIZE));
			env->sbrkAddr += (np * PAGE_SIZE);

			cprintf("\nnew sys brk:%lu\n",env->sbrkAddr - (np * PAGE_SIZE));
			return (void*) (env->sbrkAddr - (np * PAGE_SIZE));
		} else {
			return (void*)-1 ;
		}
	return (void*)-1 ;
...
/**************************************************************************/
void validate_sys_call_param(uint32 a1, uint32 a2){
	 if(!a1 || !a2 || (a1>=USER_HEAP_MAX||a1<USER_HEAP_START) ){
		 panic("\na1 = %d a2 = %d ",USER_LIMIT,USER_TOP);
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_free_user_mem:{
	  validate_sys_call_param((uint32) a1, (uint32) a2);
