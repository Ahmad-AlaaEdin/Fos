...
int ifStringIsSubsequence(char* str1, char* str2) {
    int lenofstr1 = strlen(str1);
    int lenofstr2 = strlen(str2);
    int charCounter = 0;
    for (int i = 0; i < lenofstr2 && charCounter < lenofstr1; i++)
        if (str1[charCounter] == str2[i])
        	charCounter++;
    return (charCounter == lenofstr1);
}
void clear_foundCommands_list(){
	struct Command *elem;
		LIST_FOREACH(elem,&foundCommands){
			LIST_REMOVE(&foundCommands,elem);
		}
}
	 ...
	 for (int i = 0; i < NUM_OF_COMMANDS; i++) {
		 if (strcmp(commands[i].name,arguments[0]) == 0) {
			 if(commands[i].num_of_args == (number_of_arguments-1) || commands[i].num_of_args ==-1){
				 ...
				 clear_foundCommands_list();
				 LIST_INSERT_HEAD(&foundCommands, &commands[i]);
				 return CMD_INV_NUM_ARGS;
			 }
		 }
		 else if(ifStringIsSubsequence(arguments[0],commands[i].name) == 1){
			 if(count != LIST_SIZE(&foundCommands)){
				 clear_foundCommands_list();
			 }
